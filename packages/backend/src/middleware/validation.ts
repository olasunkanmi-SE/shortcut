...
export const auctionQuerySchema = Joi.object({
  ...
  year_max: Joi.number().integer().max(new Date().getFullYear() + 1).optional(),
  ...
  sort_by: Joi.string().valid('auction_end', 'current_bid', 'created_at', 'year', 'mileage').default('created_at').optional(),
